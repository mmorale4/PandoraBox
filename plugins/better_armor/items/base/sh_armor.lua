...
function ITEM:GetDescription()
	if (self.entity) then
		return L(self.description .. "\n \n Durability:" .. math.floor(self:GetData("Durability", 5000)))
	else
...
ITEM.functions.repair = {
    ...
    OnRun = function(item)
        ...
        local number = 0
        if (character:GetAttribute("armor") >= 5) then
            for k, v in pairs(items) do
                if (v:GetName() == "Scrap Metal" and number < 5) then
                    number = number + 1
                    v:Remove()
                end
            end
if (number == 5) then
                ...
                client:Notify("You need five scrap metals to do this")
            end
        else
