...
if (SERVER) then
	...
	hook.Add( 'ScalePlayerDamage', 'Strength multipler', function( ply, hitgroup, dmginfo )
		if string.Left( dmginfo:GetInflictor():GetClass(), 11 ) == 'f4_begotten' then
			dmginfo:ScaleDamage( 1 + (ply:GetCharacter():GetAttribute("str", 0) * ix.config.Get("strengthMultiplier")) )
		end
	end)
