...
if SERVER then
	...
	function ENT:Use(user)
		...
		if not user:GetCharacter():GetInventory():HasItem("flashlight") then
			user:Notify("Get a flashlight fag")
			self:EmitSound("buttons/combine_button_locked.wav")
	...
	function ENT:Draw()
		...
		if not self:GetNetVar("InUse", false) then
			...
			draw.SimpleText("Whitelist Application Terminal", "panel_font", width / 2, 25, Color(90, 210, 255, alpha), TEXT_ALIGN_CENTER)
			draw.SimpleText("Click [use key] to begin.", "panel_font", width / 2, height - 16, Color(205, 255, 180, alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		else
