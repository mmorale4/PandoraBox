	...
	if SERVER then
		if ( self.Owner:KeyDown( IN_ATTACK ) && self.Owner:IsWepRaised() && !self:GetDTBool(3) ) then
			if self.timeCooldown < CurTime() then
				local stam = self.Owner:GetLocalVar( "agi")
				local redc = ( self.Owner:GetCharacter():GetAttribute("agi", 0) / 100 )
...
function SWEP:FireBullet( spr, ang, hit )
		...
		if SERVER then
			...
			dam = dam * ( 1 + self.Owner:GetCharacter():GetAttribute("str", 0)/100 )
			dam = dam / 3
		end
...
function SWEP:Impact( trace )
	...
	if (ix.config.Get("gathering", false) and CLIENT) then
...
function SWEP:DoDamage()
	...
	local hit
	if SERVER then
		local stam = self.Owner:GetLocalVar( "agi", 0 )
		local redc = ( self.Owner:GetChar():GetAttribute("str", 0) / 100 )
		local value = math.Clamp(stam - ( self.Primary.Stamina - redc * self.Primary.Stamina ), 0, 100)
		self.Owner:SetLocalVar( "agi", value )
	end
