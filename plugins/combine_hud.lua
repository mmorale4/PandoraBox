		...
		for k, v in pairs(player.GetAll()) do
			...
			---------------------------------------------------------------------------]]
				...
				if v:IsCombine() then
					...
					MatrixText(division .. "" .. digits, "FaceRecog", headpos.x + size / 1.9, headpos.y - size * 0.9, Color(team_color.r, team_color.g, team_color.b, alphas[v:EntIndex()]), Vector(name_size / scale, name_size / scale, 1))
					...
					if v:IsSquadLeader() then
						MatrixText("PATROL LEADER", "FaceRecog", headpos.x + size / 1.9, headpos.y - size * 1.6, Color(225, 165, 0, 255), Vector(0.05 / scale, 0.10 / scale, 1))
					end
