...
ix.command.Add("RollStat", {
	...
	OnRun = function(self, client, stat, disadvantage)
		local character = client:GetCharacter()

		if (character and character:GetAttribute(stat, 0)) then
			local bonus = (character:GetAttribute(stat, 0))
			local roll = tostring(math.random(0, (30 + (character:GetAttribute(stat,0) / 2))))
			local maxi = 30 + (character:GetAttribute(stat,0) / 2) + character:GetAttribute(stat, 0)
		...
		timer.Create(uniqueID, 0.25, 0, function()
			...
			if (current != value) then
				...
				if (value == 0 and !client:GetNetVar("brth", false)) then
					...
					character:UpdateAttrib("agi", 0.0)
