...
ix.config.Add("maxValueDurability", 1000, "Maximum value of the durability.", nil, {
	data = {min = 1, max = 1000},
		...
		if (IsValid(entity) and entity:IsPlayer()) then
			...
			if (weapon) then
				...
				for k, v in pairs(inventory) do
					if v.class == weapon:GetClass() and v:GetData("equip", true) then
						local durability = v:GetData("durability", ix.config.Get("maxValueDurability", 1000))
					
						if math.random(1, 16) == 1 and durability > 0 then
							v:SetData("durability", durability - 0.5)
						end
						...
						bullet.Damage = (bullet.Damage / 1000) * durability
		...
		v.functions.Repair = {
			...
			OnRun = function(item)
				...
				if has_remnabor then
					has_remnabor:Remove()
					item:SetData("durability", math.Clamp(item:GetData("durability", max) + 150, 0, max))
...
function PLUGIN:CanPlayerEquipItem(client, item)
	return item:GetData("durability", ix.config.Get("maxValueDurability", 1000)) > 0
end
